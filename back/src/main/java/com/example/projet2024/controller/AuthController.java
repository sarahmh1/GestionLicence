...
//    }
    ...
    public ResponseEntity<?> registerUser(@RequestBody User user) {
        ...
        // Check if the role is either CLIENT or CHEF
        if (user.getRole() != Role_Enum.ROLE_COMMERCIAL && user.getRole() != Role_Enum.ROLE_MANAGER) {
            return ResponseEntity.badRequest().body("Error: Invalid role selected! Only 'Commercial' or 'manager' roles are allowed.");
        }
